...
object ResourceUtils {
     ...
     * 解析为表
     */
    inline fun <reified K, reified V> resolveToMap(resource: NexaResource, type: ConfigUtils.ConfigType) = resolveToMap<K, V>(resource.contentAsByteArray(), type)
     ...
     * 写入对象到输出流
     */
    fun <T> writeObject(outputStream: OutputStream, type: ConfigUtils.ConfigType, content: T) = outputStream.write(type.write(content))

    @Suppress("UNCHECKED_CAST")
    fun flattenStringMap(map: Map<String, Any?>, parentKey: String = "", result: MutableMap<String, String> = mutableMapOf()): Map<String, String> {
