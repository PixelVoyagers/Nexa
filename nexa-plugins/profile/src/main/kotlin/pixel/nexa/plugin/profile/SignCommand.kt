...
class SignCommand(private val userExperienceHandler: UserExperienceHandler, private val signHandler: SignHandler) : NexaCommand() {
        ...
        session.replyLazy {
            if (canSign) {
                ...
                MutableMessageData().add(
                    translatableReply("success", rewards.joinToString("\n") { "    " + it.getDisplay().asText(user.getLanguageOrNull() ?: session.getLanguage()) })
                )
            } else {
                val duration = Duration.parse("1d") - between!!.abs().toKotlinDuration()
                MutableMessageData().add(
                    translatableReply("failed", duration.toJavaDuration().truncatedTo(ChronoUnit.SECONDS).toKotlinDuration())
                )
            }
        }
    }
